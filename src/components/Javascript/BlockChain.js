        ...
        return(
            ...
            >
                ...
                <Block margin={`${remCalc(20)} 0`}>
                    Source code: <Link target="_blank" rel="noopener noreferrer" href="https://github.com/nialloc9/blockChain">GitHub</Link>
                </Block>

                <Block margin={`${remCalc(20)} 0`}>
                    Block chain has really been hitting the news recently with the success of crypto technologies such as bitcoin. However, what is not as well known is that block chain
                ...
                <Block margin={`${remCalc(20)} 0`}>
                    So how does block chain actually work? Well at it's simplest it works by chaining together a group of blocks. And after being chained the block cannot be modified without modifying the whole chain. This concept is
...
`}
                ...
                <Block margin={`${remCalc(20)} 0`}>
                    From above we can see that a block in a block chain has attributes that needed to be passed into the constructor to create a block. The only optional one
                ...
                <Block margin={`${remCalc(20)} 0`}>
                    Notice in the mineBlock method that we pass in a difficulty paramater. This is interesting because it is used to determin the number of 0's required at the front of the hash to
...
`}
                ...
                <Block margin={`${remCalc(20)} 0`}>
                    As we can see from the constructor a genesis block is created in which will be the first block in the chain. It all takes a difficulty level which will
