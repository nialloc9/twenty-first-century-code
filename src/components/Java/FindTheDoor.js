    ...
    render(){
        return(
            ...
            >
                ...
                <Block margin={`${remCalc(20)} 0`}>
                    This is a 2d game made using the slick2d library. The object of the game is to find a secret door in the wall.
                ...
                <Block margin={`${remCalc(20)} 0`}>
                    The super simple super awesome java 2d slick game is a two state game. States are basically different screens and display different things when selected. Usually a game would have many states such as one for the main menu,
                ...
                <Block margin={`${remCalc(20)} 0`}>
                    To create a state using the slick 3d library there are three steps to take:
                </Block>

                <Block margin={`${remCalc(20)} 0`}>
                    <Block>
                        <strong>1:</strong> Give it a reference number(will be used when changing states)
                        <strong>2:</strong> Add it as a state to the StateBasedGame object
                        <strong>3:</strong> Initialize the state
                    </Block>
                </Block>
...
`}
                ...
                <Block margin={`${remCalc(20)} 0`}>
                    As you can see from the code above inside our Main class we have two methods and a {`constructor`}. The main class inherits from the StateBasedGame class that we imported from the state package.
                ...
                <Block margin={`${remCalc(20)} 0`}>
                    Let’s look inside the {`constructor`} first. The {`constructor`} takes one parameter.  The name of the game to be created. For this game the name is “Super Simple Super Awesome”. It also creates two new states
                ...
                <Block margin={`${remCalc(20)} 0`}>
                    The initStatesList is an overridden method from the StateBasedGame class initializes the states and enters the main state upon starting the game while the main method creates a new instance of our game and adds
...
`}
            ...
            <Block margin={`${remCalc(20)} 0`}>
                The states that we create are the menu and the play states. To create a state for the game we override some methods from the BasicGameState class it inherited from. First the getId method which returns the state
...
`}
                ...
                <Block margin={`${remCalc(20)} 0`}>
                    The play state is similar to the main state in that it overrides the same classes because it inherits from basicGameState too. If you look at the data members there is a quit variable. As long as this is false the
