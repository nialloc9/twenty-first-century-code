    ...
    render(){
        return(
            ...
            >
                ...
                <Block margin={`${remCalc(20)} 0`}>
                    This project is actually a combination of two projects. First a server side project and second a client side project. Because the server side is waiting
...
`}
                ...
                <Block margin={`${remCalc(20)} 0`}>
                    As you can see it creates an instance of Server. Because Server is a child of the class JFrame we give it a default close operation. If we didn’t we would never be able to close it. And lastly we put the server
...
public class Server extends JFrame{
  ...
  `}
                ...
                <Block margin={`${remCalc(20)} 0`}>
                    To allow the user to chat we first need a graphical user interface. We extend JFrame to inherit it’s properties and methods. We then create some swing components for the user to view and output data.
                ...
                <Block margin={`${remCalc(20)} 0`}>
                    Now let’s look at the startRunning method that we briefly spoke about earlier. Inside this we set up our socket. We assign the socket a port number of 6789 and a backlog (queue length) of 100. The backlog is the
                ...
                <Block margin={`${remCalc(20)} 0`}>
                    To set up a stream we need to first create a try, catch, finally block to handle an end of stream exception that may occur. We added the finally here so we can close off our stream at the end regardless if there is an exception or not.
                ...
                <Block margin={`${remCalc(20)} 0`}>
                    Let’s now look at what happened inside the try, catch, finally block. Three methods were called. The first waitForConnection waits for a user to try and connect and when they do calls the accept method from the server object to put the connection on a socket.
                ...
                <Block margin={`${remCalc(20)} 0`}>
                    Now that the streams and sockets are all set up it’s time to look at the whileChatting method. First we output a message to the screen that it is time to chat and we also change the input JTextfield editable to true to allow for input.
                ...
                <Block margin={`${remCalc(20)} 0`}>
                    Remember the closeCrap method? Well now let’s look at it. See how it closes all our streams and sockets? This is important because we don’t want to leave these open when the chat is closed. Also these must be wrapped in a try catch
                    because they can throw an IO exception when attempting to close.
                </Block>

                <Block margin={`${remCalc(20)} 0`}>
                    When sending a message we use the writeObejct method that throws an IOException so must be wrapped in a try catch to handle it. Again here we flush it after sending to make sure no data is left in the buffer. Now we show the message using
                ...
                <Block margin={`${remCalc(20)} 0`}>
                    Now let’s check out what’s happening in the other project, the client side and see how it communicates with the server side. This will be a much shorter description as a lot of the principles from the server side apply to client
...
`}
                ...
                <Block margin={`${remCalc(20)} 0`}>
                    I enjoyed making this project to make use of sockets and streams and to demonstrate a practical use for them. It really is a good way to understand how they work and it is awesome to see.
